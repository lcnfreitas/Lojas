...
class AddProductToStore extends Component {
    ...
    handleSubmit = async event => {
        ...
        if (product_name === '' || product_name.includes(",")) {
            swal("Ops!", "Nome do Produto é um campo obrigatório e não deve conter vírgulas.", "error");
        ...
        if (product_name !== '' && product_price !== '' && this.isValidFloat(product_price) === true && product_qty !== '') {
            try {
                 // Adiciona o novo produto na tabela de produtos
                ...
                if (product_insert_response.status === 200) {
                    swal("Sucesso!", "Novo produto adicionado com sucesso!", "info")
                    .then((value) => {
                ...
                console.log(e);
                swal("Ops!", "Ocorreu um erro ao adicionar este produto.", "error");
            }
        }
    }
        ...
        return (
            <div className="wrapper">
                <form className="container-fluid" encType="multipart/form-data">
                    <h2 className="header">Adicionar Novo Produto ao Estoque Global</h2>
                    ...
                    <div className="add_store_action_btn_block">
                        ...
                        <Link to={`/products`}><button className="btn btn-outline-danger action_btn">Cancelar</button></Link>
                    </div>
                </form>
            </div>
        )
